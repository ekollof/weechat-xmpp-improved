...
- ⚡ XEP-0413: Order-By for MAM (Experimental — `urn:xmpp:order-by:1 field='creation-date'` included in XEP-0442 pubsub MAM queries for newest-first ordering; advertised in caps)
 ...
 - ⚡ XEP-0448: Encrypted File Sharing (Experimental — send: when `/upload`-ing a file to an OMEMO-enabled channel, the file is AES-256-GCM encrypted before HTTP PUT; the XMPP message carries `<file-sharing>` with `<encrypted xmlns='urn:xmpp:esfs:0' cipher='urn:xmpp:ciphers:aes-256-gcm-nopadding:0'>` containing the base64 key/IV/hash; receive: `<encrypted xmlns='urn:xmpp:esfs:0'>` detected inside SFS `<sources>`, ciphertext downloaded in a background thread, decrypted with AES-256-GCM, saved to `~/Downloads/<filename>`; `urn:xmpp:esfs:0` advertised in caps)
 - ⚡ XEP-0434 / XEP-0450: Trust Messages + Automatic Trust Management (Experimental — TOFU: on first OMEMO session establishment a SHA-256 fingerprint of the identity key is stored as `trusted` in LMDB; an unencrypted `<trust-message xmlns='urn:xmpp:tm:1' usage='urn:xmpp:atm:1'>` is sent to own bare JID to propagate trust to other own endpoints; incoming trust messages are parsed and applied to the local trust store, overriding plain TOFU for subsequent `identity_is_trusted` checks; configurable via `xmpp.look.omemo_atm on/off`; `urn:xmpp:tm:1` advertised in caps)
